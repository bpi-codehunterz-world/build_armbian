...
Main() {
	case $RELEASE in
		...
		bullseye)
			apt-get update
			apt-get install -y gcc clang build-essential software-properties-common python3-dev python3-pip python3-venv
			apt-get install -y libc6-dev
			apt-get install -y htop ufw nodejs npm ruby-dev bash-completion nginx-full aircrack-ng net-tools traceroute whois nmap
			apt-get install -y php php-fpm php-cgi php-curl php-mysql php-zip
